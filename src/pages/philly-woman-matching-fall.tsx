...
export default function PhillyWomanMatchingFall() {
    ...
    useEffect(() => {
        const pullPlayers = async () => {
            await fetch("/api/pullPlayers", {
                ...
                body: JSON.stringify({league: "phillywomanmatchingfall"})
            })
    ...
    const handleSubmit = async (playersLst: string[]) => {
        ...
        await fetch("/api/pullSchedules", {
            ...
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({
                league: "phillywomanmatchingfall",
        ...
        await fetch("/api/generateSchedule", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({league: "phillywomanmatchingfall", playersLst: playersLst})
        })
            .then((res) => {
                if (res.ok) {
                    return res.json()
    ...
    return (
        <div ref={contentRef}>
            ...
            <div className={`${popup !== "" ? "blur-sm" : ""}`}>
                ...
                <div>
                    <InputNames league="2023 Philly Woman-Matching Fall" handleSubmit={handleSubmit} scheduleGenerated={scheduleGenerated} readyToGenerate={readyToGenerate} errorType={errorType} />
                </div>
                <div>
