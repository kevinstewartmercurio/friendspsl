...
import { uhle2023RosterUrls } from "@/rosterUrls"
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const oldPlayers = await players.find({league: "uhle"})
    if (oldPlayers) {
        ...
        const oldDate = await oldRangeDatePromise
    ...
    for (let i = req.body.start; i <= req.body.end; i++) {
        for (let name of (await rosterUrlToNamesLst(uhle2023RosterUrls[i - 1]) as string[])) {
    ...
    await players.updateOne({league: "uhle"}, updateRangePlayersQuery)

    const updateRangeDateQuery = {$set: {} as {[key: string]: Date}}
    updateRangeDateQuery.$set[`date-${req.body.start}-${req.body.end}`] = new Date()
    await players.updateOne({league: "uhle"}, updateRangeDateQuery)
    

    console.log(`updated uhle ${req.body.start}-${req.body.end}`)
    return res.status(200).json({})
}