...
export default async function handler(
    ...
    if (oldPlayers) {
        const oldRangeDatePromise = oldPlayers
            .toArray()
            .then((docs: any) => {
                if (docs[0] !== undefined) {
    ...
    for (let i = req.body.start; i <= req.body.end; i++) {
        for (let name of (await rosterUrlToNamesLst(
            leagueToRosterUrls[req.body.league][i - 1]
        )) as string[]) {
            rangePlayersObj[name] = i;
        }
    }

    const updateRangePlayersQuery = {
        $set: {} as { [key: string]: { [key: string]: number } },
    };
    updateRangePlayersQuery.$set[`players.${req.body.start}-${req.body.end}`] =
        rangePlayersObj;
    await players.updateOne(
        { league: req.body.league },
        updateRangePlayersQuery
    );

    const updateRangeDateQuery = { $set: {} as { [key: string]: Date } };
    updateRangeDateQuery.$set[`date-${req.body.start}-${req.body.end}`] =
        new Date();
