...
export default function DelawareFallOpen() {
    ...
    useEffect(() => {
        const pullPlayers = async () => {
            await fetch("/api/pullPlayers", {
                ...
                body: JSON.stringify({league: "delawarefallopen"})
            })
    ...
    const handleSubmit = async (playersLst: string[]) => {
        ...
        await fetch("/api/generateSchedule", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({league: "delawarefallopen", playersLst: playersLst})
        })
            .then((res) => {
                if (res.ok) {
                    return res.json()
    ...
    return (
        <div ref={contentRef}>
            ...
            <div className={`${popup !== "" ? "blur-sm" : ""}`}>
                ...
                <div>
                    <InputNames league="2023 Delaware Fall Open" handleSubmit={handleSubmit} scheduleGenerated={scheduleGenerated} readyToGenerate={readyToGenerate} errorType={errorType} />
                </div>
                <div>
